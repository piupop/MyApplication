package com.marimari.eventremindlist.eventreminder;
...
import com.marimari.eventremindlist.eventreminder.model.MyModel;
...
public class AddeventActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reEditId = intent.getIntExtra("idToEdit", -1);//intentがないとき
    ...
    public void add(View v){
        ...
        if ( reEditId == -1) {
            ...
            realm.executeTransaction(new Realm.Transaction() {
                @Override
                public void execute(Realm bgRealm) {
                    MyModel result2 = bgRealm.getDefaultInstance().where(MyModel.class).equalTo("id", reEditId).findFirst();
                    String date1Text = editDate1.getText().toString();
                    String date2Text = editDate2.getText().toString();
                    String titleText = editTitle.getText().toString();
                    ...
                    result2.setDate1(date1Text);
                    result2.setDate2(date2Text);
                    result2.setTitle(titleText);
                    result2.setRate(rateNum);
                    result2.setMemo(memoText);
                    finish();
                }
            });
        }
